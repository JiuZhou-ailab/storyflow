...
$process = Get-Process -Name "Storyflow" -ErrorAction SilentlyContinue
if ($process) {
    Write-Info "Closing Storyflow..."
...
Write-Info "Adding 'storyflow' command to PATH..."

$binDir = "$env:LOCALAPPDATA\Storyflow\bin"
$cmdFile = "$binDir\storyflow.cmd"
$exePath = "$env:LOCALAPPDATA\Programs\Storyflow\Storyflow.exe"
...
if ($userPath -notlike "*$binDir*") {
    ...
    Write-Success "Added to PATH (restart terminal to use 'storyflow' command)"
} else {
    Write-Success "Command 'storyflow' is ready"
}
...
Write-Host ""
Write-Host "  Storyflow has been installed."
Write-Host ""
Write-Host "  Launch from:"
Write-Host "    - Start Menu or desktop shortcut"
Write-Host "    - Command line: storyflow (restart terminal first)"
Write-Host ""
}
