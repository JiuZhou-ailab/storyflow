...
const existingEndpoint = process.argv.find(arg => arg.startsWith('--endpoint='))?.slice('--endpoint='.length)
...
while (true) {
  ...
  try {
    const [port, path] = existingEndpoint ? [] : readFileSync(join(profile, 'DevToolsActivePort'), 'utf8').trim().split(/\r?\n/)
    const endpoint = existingEndpoint ?? `ws://127.0.0.1:${port}${path}`
    ...
    console.log(JSON.stringify({ ...result, endpoint }))
