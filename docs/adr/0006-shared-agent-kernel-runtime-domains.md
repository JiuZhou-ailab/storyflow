...
状态：Accepted（2026-07-23）；Skills 与本地文件权限部分由 ADR 0010 修订
日期：2026-07-23

## 背景

Storyflow 需要同时支持应用级自由对话和项目级写作对话。两者必须隔离历史、默认工作目录和 UI 上下文，
但继续复用相同的 Agent 能力、全局 Skills 和本机用户权限。当前会话已经按 `workspaceRootPath` 存储，
...
- 自由对话使用稳定的隐藏 workspace id，项目对话使用项目 workspace id。会话存储、默认工作目录
  和 UI 资源视图从这个 workspace 事实源推导；它不是本地文件授权边界。
- 自由对话通过应用内部的隐藏 Workspace 复用现有 workspace-scoped session
  持久化；它不出现在项目列表中，每个自由对话使用独立工作目录。
- 项目对话继续使用现有项目 workspace 和原有会话数据，不迁移历史内容。
- 全局资源的唯一根是 `~/.craft-agent`。Skills 和 Extensions 只允许全局安装；Sources 保留
  项目覆盖层。模型连接继续由现有全局配置与会话引用机制选择。
- `<project>/.pi/skills/` 不再参与加载。旧目录保留不动，不自动合并到全局目录。
- 交互式 Agent 默认使用 `allow-all`，本地文件能力与启动 Storyflow 的操作系统用户一致；
  Project 只提供默认 cwd 与上下文，不提供授权。用户仍可显式切换 `safe` 或 `ask`。
...
- 自由对话和项目对话在 UI 与历史上具有可验证的所有权边界，同时共享本机用户文件权限。

代价：

- 隐藏 workspace 解析处必须处理一个明确的自由对话 id。
- Prompt 与文件树读取当前 Workspace 作为默认上下文；工具权限不从 Workspace 推导。
