 ...
 *
 * This module is runtime-neutral. SDK-specific tool wrappers live beside their
 * adapters so importing the Pi protocol never loads another agent framework.
 */

import { readFile } from 'fs/promises';
import { existsSync, statSync } from 'fs';
import path from 'node:path';
import { getModelById, MODEL_REGISTRY } from '../config/models.ts';
...
export const MAX_TOTAL_CONTENT_BYTES = 2_000_000; // 2MB total across all attachments
