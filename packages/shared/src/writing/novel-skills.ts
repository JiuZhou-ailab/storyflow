...
const OH_STORY_SKILLS = [
  adapterSkill({
    ...
    context: ["全局/ 存放 canon。", "大纲/ 存放整本书和章节规划。", "追踪/ 存放连续性、伏笔和时间线状态。"],
  ...
  adapterSkill({
    ...
    workflow: ["不要从第一个模糊请求直接起草，即使用户说“写一个”。", "从提示词中抽取已知事实：前提、设定、主角、关系、冲突、情绪引擎、预期兑现、篇幅和平台线索。", "识别会决定方法的缺失维度：男频/女频/无CP/双男主/双女主、题材赛道、打脸/逆袭/复仇/种田/悬疑/甜宠引擎、腹黑或直给人设、POV、反转节奏、结局形态、禁区约束，以及用户想先要大纲、黄金三章、分章还是正文。", "只提出阻塞问题；低风险缺失项可以明确给出默认值。", "分流到调研、拆文、长篇写作或短篇写作前，先写入或更新 全局/题材定位.md 和 大纲/大纲.md。"],
  ...
  adapterSkill({
    ...
    context: ["调研材料存放在 参考资料/。", "对标列表和笔记存放在 对标/ 或 拆文库/。", "趋势笔记只有被接受进 全局/ 或 大纲/ 后才算 canon。"],
    workflow: ["定义目标平台和题材赛道。", "收集可比作品、开篇钩子、更新模式和读者承诺。", "总结可长期使用的定位约束，不复制剧情。"],
    output: ["参考资料/<topic>.md 调研笔记。", "选定对标时的 对标/<title>/ 笔记。", "写入 全局/题材定位.md 的定位建议。"],
  }),
  adapterSkill({
    ...
    context: ["源文本摘录放在 对标/ 或 拆文库/。", "分析报告要区分证据和解释。", "原创项目 canon 仍然保存在 全局/。"],
  ...
  adapterSkill({
    ...
    context: ["全局/ 是 canon。", "大纲/ 是章节契约。", "正文/ 存放已接受章节。", "每章接受后更新 追踪/。"],
    workflow: ["起草前确认 全局/题材定位.md 和 大纲/大纲.md 非空。", "在 大纲/ 中创建或更新章节简报。", "先在 .work/ 起草，再把已接受文本写入 正文/。", "接受后更新 追踪/上下文.md、追踪/伏笔.md 和 追踪/时间线.md。"],
  ...
  adapterSkill({
    ...
    context: ["短篇仍然使用 全局/、大纲/、正文/ 和 追踪/。", "调研材料放在 参考资料/。"],
  ...
  adapterSkill({
    ...
    context: ["对照 全局/、大纲/ 和 追踪/。", "审校报告放在 .work/。"],
  ...
  adapterSkill({
    ...
    context: ["用 全局/题材定位.md 和 大纲/ 确认故事承诺。", "生成资产或提示词不要写入 canon 文件。"],
