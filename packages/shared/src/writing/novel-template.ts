...
- 第一章必须有首章入场坡道：原创题记或引文、开场第一镜头、前三段钩子和读者疑问。
- 默认一次只写一章；不要一次生成多章或多篇正文，除非用户明确要求批量生成。
- 初始请求：先把题材定位、主角设置、目标读者、核心钩子、首章入场坡道、黄金三章、小说密度、事件密度和情绪调动程度写进 简报.md，再补 大纲.md，最后才进入 正文/。

## 文件契约

- \`创作要求.md\`：跨项目长期生效的写作风格、首章头部偏好、读者偏好、禁区。
- \`简报.md\`：当前作品的题材、卖点、目标读者、核心钩子、首章入场坡道、黄金三章、小说密度、事件密度、情绪调动程度、篇幅目标。
- \`大纲.md\`：分章节 beat 计划，每章一段，首章必须包含题记 / 引文和前三段推进。
...
- 在 简报.md 的首章入场坡道、黄金三章、小说密度、事件密度或情绪调动程度仍缺失时，不要写入 正文/；前三章留存设计不清楚，后续大纲先停。
- 先确认题材定位、主角设置、目标读者、核心钩子、首章题记或引文方向、前三章留存目标、小说密度、事件密度、情绪调动程度和篇幅目标，把答案落到 简报.md 与 大纲.md，再进入正文。
...
- 默认中文第一人称口语化叙事，章节标题就是钩子；第一章不要冷启动开讲，先用原创题记、章首引文或一句带刺自白建立情绪命题，再落入具体冲突。
- 前三章按留存功能设计：第 1 章拉新，第 2 章加压，第 3 章锁留存；不要把关键承诺拖到三章以后才出现。
- 章首题记 / 引文默认原创；不要伪造出处，不使用不可核验名人名言。前三段依次完成情绪命题、异常现场、读者疑问。
...
- 题记 / 引文偏好（默认原创，不伪造出处，不使用不可核验名人名言）：
- 是否允许一句带刺自白代替题记：
- 第一镜头偏好（场景 / 物件 / 对话 / 身体反应 / 事故现场）：
- 前三段必须建立的读者疑问：
