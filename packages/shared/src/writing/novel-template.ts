...
- \`全局/\`：世界观、角色、势力、关系、术语和故事约束。
- \`全局/题材定位.md\`：市场赛道、读者承诺、题材期待、语气和差异化钩子。
...
- \`全局/项目说明.md\`：当前项目目的、范围和轻量约定。
- \`全局/创作要求.md\`：长期写作偏好与禁区。
- \`正文/\`：已接受正文或最终创作产物。
...
- \`全局/创作要求.md\`：长期偏好与禁区，跨当前作品生效。
- \`全局/简报.md\`：当前作品 intake，记录本书的输入约束和已确认方向。
- \`全局/大纲.md\`：章节 outline，正文写作前的结构依据。
- \`全局/人物.md\`：角色 reference。
- \`正文/\`：accepted prose，唯一成稿目录。
- \`自由区/\`：scratch，用于试写、临时方案、审校笔记和废弃版本。

## 写入边界

- 在 \`全局/简报.md\` 与 \`全局/大纲.md\` 仍是模板或缺少当前作品有效内容时，不要写入 \`正文/\`。
- 章节先有 \`全局/大纲.md\` 对应段落，再写 \`正文/NN-标题.md\`。
- 修订直接覆盖同一章节文件，让 git diff 承担版本历史；不要新建 \`草稿/\` 或 \`定稿/\` 目录。
- 实验、被废弃的章节版本、试写和审校笔记放 \`自由区/\`，不要污染 \`正文/\`。
- 正文/ 可以按卷、篇或阶段建立子目录；全局/ 放简报、大纲、人物和长期设定；自由区/ 可以自由创建文件和文件夹。
...
function scaffoldOhStory(rootPath: string): void {
  ...
  writeFileIfMissing(join(rootPath, "全局/关系.md"), `# Relationships

## Character Map
`);
  writeFileIfMissing(join(rootPath, "全局/题材定位.md"), `# Genre Positioning
...
function scaffoldFreeCreation(rootPath: string): void {
  ensureDir(join(rootPath, "全局"));

  writeFileIfMissing(join(rootPath, "全局/项目说明.md"), `# 项目说明
...
export function createNovelProjectScaffold(
  ...
  writeFileIfMissing(getWorkspacePackLockPath(rootPath), createPackLock(pack));
  writeFileIfMissing(getWorkspaceAgentsPath(rootPath), createAgentInstructions(pack));
  writeFileIfMissing(getWorkspaceClaudePath(rootPath), createAgentInstructions(pack));
  writeFileIfMissing(getWorkspaceNoticePath(rootPath, getNoticeFileName(pack)), createNotice(pack));

  for (const file of getBundledNovelSkillFiles(pack.id)) {
    writeFileIfMissing(join(getWorkspaceSkillsPath(rootPath), file.relativePath), file.content);
  }
}
