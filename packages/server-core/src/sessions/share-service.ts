// input: Session registry lookups, stored-session reads/metadata writes, viewer endpoint, and the broadcaster
// output: Share-to-viewer lifecycle (create/update/revoke) with async-operation UI signaling
// pos: Session sharing subdomain under the SessionManager facade; owns no state beyond its injected deps
...
export class ShareService {
  constructor(private deps: ShareServiceDeps) {}
  ...
  async shareToViewer(sessionId: string): Promise<ShareResult> {
    ...
    this.deps.broadcaster.sendEvent({ type: 'async_operation', sessionId, isOngoing: true }, managed.workspace.id)
    ...
    try {
      ...
      this.deps.broadcaster.sendEvent({ type: 'session_shared', sessionId, sharedUrl: data.url }, managed.workspace.id)
      ...
      this.deps.broadcaster.sendEvent({ type: 'async_operation', sessionId, isOngoing: false }, managed.workspace.id)
    }
  }
  ...
  async revokeShare(sessionId: string): Promise<ShareResult> {
    ...
    try {
      ...
      // Notify all windows for this workspace
      this.deps.broadcaster.sendEvent({ type: 'session_unshared', sessionId }, managed.workspace.id)
      return { success: true }
    } catch (error) {
