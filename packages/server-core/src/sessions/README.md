...
| `plan-tracking.ts` | Pending plan execution storage wrappers (five) |
...
| `source-bridge.ts` | buildServersFromSources / applyBridgeUpdates shared by source reload paths |

Pre-existing support modules: `managed-session.ts` (ManagedSession state assembly), `runtime-config.ts` (runtime/restart signatures), `pi-turn-anchors.ts` (provider fork anchors), `tool-display.ts` (UI tool projection), `turn-watchdog.ts` (absolute turn safety limit), `managed-gateway-auth-error.ts`, `write-original-content.ts`.

Module wiring rule: every module receives a narrow deps object of arrow functions resolving through the Facade (`this`) at call time, so per-instance test stubs on SessionManager keep working. Adjacent `*.test.ts` files verify these boundaries and durable session behavior.
