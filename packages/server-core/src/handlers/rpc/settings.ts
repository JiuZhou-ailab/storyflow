...
export function registerSettingsHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.systemInstructions.READ, async () => {
    const path = getSystemInstructionsPath()
    try {
      if (!existsSync(path)) {
        return { content: '', exists: false, path }
      }
  ...
  server.handle(RPC_CHANNELS.systemInstructions.WRITE, async (_, content: string) => {
    try {
      saveSystemInstructionsMarkdown(content)
      return { success: true }
    } catch (error) {
      return { success: false, error: error instanceof Error ? error.message : 'Unknown error' }
    }
