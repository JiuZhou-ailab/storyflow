...
- `GET /v2/video-assets`：按单一来源、剧目 ID 或标题分页查询视频；ReelShort 标题查询额外返回实时 `seriesSearch` 候选，MP4 返回 `downloadUrl`，HLS 返回 `playbackUrl` 与 `downloadMethod=hls_remux`。
...
部署可通过 `REELSHORT_SEARCH_URL` 覆盖 ReelShort 实时搜索服务地址；未配置时不扩展数据库内已有结果。
