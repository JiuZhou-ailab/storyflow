...
- **扩展托管模型列表** — 内置 `JiuZhou-AI` 继续默认走 Cloudflare AI Gateway `custom-wangsu`，默认模型改为 `gpt-5.5`，仍可选 `gemini-3.5-flash`、`deepseek-v4-pro`；同时新增隐藏托管连接 `xiaomi-default`，可在聊天模型菜单选择 `mimo-v2.5`。
- **拆分登录与模型网关凭证** — Feishu/Neon 登录现在通过 auth broker 换取托管模型网关凭证，桌面端不会保存 Feishu app secret；托管连接凭证会同步写入 `wangsu-default` 与 `xiaomi-default`。新增 Cloudflare Worker 模型代理，用于后续把桌面侧短期登录 JWT 与服务端 Cloudflare AI Gateway token 分离。
