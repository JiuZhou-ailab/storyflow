...
这一版新增更新后公告弹窗，优化新对话开场入口，并内置默认写作资源；更新后首次进入客户端会直接看到简短说明。
...
- **强制新用户先登录** — 正式桌面包默认启用客户端登录门禁，新安装用户打开 App 后会先进入登录页；发布构建会校验 Feishu 与 Neon 两种登录入口的必要配置，避免漏配后直接进入主界面。
- **修复新安装后的更新与登录配置** — 首次打开或刚完成版本切换时不再自动下载更新并提示重启；发布包会正确读取编译进 bundle 的 Feishu/Neon 登录配置，避免更新后显示“客户端鉴权未配置”。下载页链接也改为稳定 latest 资产，避免网页未部署时继续下载旧包。
- **修复中文输入与多窗口登录状态** — 聊天输入默认不再自动大写首字母，避免破坏中文拼音 IME 的首键输入；一个窗口登录或退出后，其他已打开窗口也会同步刷新登录状态。
- **修复会话分支与编辑回放上下文** — 会话重载后会保留消息能否安全分支的信息；编辑首条用户消息重新生成时，会继续使用原会话的模型连接和模型，而不是静默切回全局默认值。
...
- **新增更新后公告弹窗** — 桌面端完成版本更新后，首次进入客户端会弹出简短的更新说明，直接说明这一版更新了什么；用户关闭后不再重复弹出，也可以从左侧“更新”入口查看完整更新内容。
- **优化新对话开场入口** — 空会话开场按钮改按写作、资料和工具分组展示，并补充更短的说明文案；窄侧栏里的模型选择和发送按钮也会优先保持可见，减少按钮被长模型名挤掉的问题。
- **内置默认写作资源** — 桌面端启动时会把内置写作 Skills 和 Wangwen BigData 数据源种子复制到 `~/.agents/`，仅补齐缺失资源，不覆盖用户已经修改过的技能或数据源配置。
- **补充工作区结构上下文** — Agent 每轮会收到一个有边界的当前工作区文件树，帮助它区分真实文件路径和展示名称，减少凭空编造路径或忽略已有项目文件的情况。
