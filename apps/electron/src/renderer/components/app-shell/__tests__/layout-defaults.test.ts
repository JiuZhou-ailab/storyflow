...
const workspaceDockSource = readFileSync(new URL('../../workspace/WorkspaceDockLayout.tsx', import.meta.url), 'utf8')
...
describe('app shell layout defaults', () => {
  ...
  it('uses the existing panel headers as window chrome without a duplicate project title strip', () => {
    ...
    expect(appShellSource).toContain('rightSidebarButton={conversationWorkspace.button ?? (isAutoCompact || !rightWorkspaceVisible ? rightWorkspaceToggleButton : undefined)}')
  ...
  it('folds the directory inside a stable shared writing workspace header', () => {
    ...
    const directoryColumnStart = workspaceDockSource.indexOf('<ResizableColumn')
    const directoryColumnEnd = workspaceDockSource.indexOf('</ResizableColumn>', directoryColumnStart)
    expect(workspaceColumnStart).toBeGreaterThan(-1)
    expect(directoryColumnStart).toBeGreaterThan(-1)
    expect(appShellSource.indexOf('<WorkspaceDockLayout', workspaceColumnStart)).toBeGreaterThan(workspaceColumnStart)
    expect(directoryColumnEnd).toBeGreaterThan(directoryColumnStart)
    const directoryColumnSource = workspaceDockSource.slice(directoryColumnStart, directoryColumnEnd)
    ...
    expect(appShellSource).toContain('directoryWidth={workspaceDirectoryWidth}')
    expect(directoryColumnSource).toContain('width={directoryWidth}')
    ...
    expect(appShellSource).toContain("onCreateFile={() => handleWorkspaceOpeningCommand('create-file')}")
    expect(novelDocumentTabStripSource).toContain("t('writing.createFile.menu'")
    expect(novelDocumentTabStripSource).not.toContain('chatOpening.project.emptyTitle')
