...
describe('project management entry', () => {
  ...
  it('keeps global navigation usable from the project manager when an active project exists', () => {
    ...
    expect(appSource).toContain('onOpenWhatsNew: handleOpenRuntimeWhatsNew')
    expect(appSource).toContain('whatsNew: { unseen: hasUnseenReleaseNotes }')
    expect(appSource).toContain('runtimeNavigationWorkspaceId ?? FREE_CONVERSATION_WORKSPACE_ID')
    expect(appSource).toContain('openWhatsNewSignal={openWhatsNewSignal}')
    expect(appSource).toContain('whatsNewManifest={whatsNewManifest}')
    expect(appSource).toContain('hasUnseenReleaseNotes={hasUnseenReleaseNotes}')
    expect(appSource).toContain('onReleaseNotesSeen={() => setHasUnseenReleaseNotes(false)}')
