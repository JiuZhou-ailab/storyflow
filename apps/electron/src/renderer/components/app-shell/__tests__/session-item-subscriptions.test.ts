...
describe('session item subscriptions', () => {
  ...
  it('keeps focus actions off focus-state context updates', () => {
    ...
    expect(focusContextSource).toContain("document.activeElement?.closest<HTMLElement>('[data-focus-zone]')")
    expect(focusContextSource).toContain('getDomFocusZone() ?? focusStateRef.current.zone')
    expect(focusContextSource).toContain('getAdjacentZoneId(currentZone')
    expect(focusContextSource).toContain('getMountedZoneIds')
    expect(focusContextSource).toContain('zone?.ref.current?.isConnected')
    expect(appShellSource).toContain("useAction('nav.previousZone'")
    expect(appShellSource).toContain('function hasBlockingFocusDialog()')
    expect(appShellSource).toContain('[data-slot="dialog-content"]')
    expect(appShellSource).toContain('[role="dialog"]:not([data-slot="popover-content"])')
  ...
  it('reuses a label lookup map instead of scanning labels per row badge', () => {
    ...
    expect(sessionItemSource).toContain('onLabelsChange={ctx.onLabelsChange}')
    ...
    expect(sessionBadgesSource).not.toContain('useSessionListContext')
  ...
  it('keeps whole-session metadata subscriptions out of the app shell', () => {
    ...
    const globalSearchContentSource = globalSearchDialogSource.slice(
      globalSearchDialogSource.indexOf('function GlobalSearchDialogContent'),
      globalSearchDialogSource.indexOf('function GlobalSearchResults')
    )
    expect(globalSearchContentSource).not.toContain('useAtomValue(sessionMetaMapAtom)')
  })
