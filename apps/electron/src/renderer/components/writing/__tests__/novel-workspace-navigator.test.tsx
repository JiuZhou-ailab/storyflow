...
describe('novel writing workspace layout', () => {
  ...
  it('rewrites selected novel text in place without routing through the chat stream', () => {
    ...
    expect(appShellSource).toContain("type: 'rewriteNovelSelection'")
    ...
    expect(appShellSource).not.toContain('window.electronAPI.rewriteNovelSelection')
  ...
  it('auto-saves before file switching and selection Ask AI when the current document has unsaved edits', () => {
    ...
    expect(askAiSource).toContain("type: 'rewriteNovelSelection'")
    expect(askAiSource).not.toContain('window.electronAPI.rewriteNovelSelection')
