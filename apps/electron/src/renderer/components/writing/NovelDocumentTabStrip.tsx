...
export function NovelDocumentTabStrip({
  ...
  return (
    <div
      ...
      className="titlebar-drag-region relative z-panel flex h-[42px] shrink-0 items-stretch border-b border-foreground/[0.10] bg-background"
    >
        ...
        {files.map((file) => {
          ...
          return (
            <div
              key={file.path}
              className={cn(
                'titlebar-no-drag group relative flex min-w-[112px] max-w-[200px] shrink-0 items-center border-r border-foreground/[0.10] text-xs',
            ...
            >
              ...
              >
                <FileText className="h-3.5 w-3.5 shrink-0 opacity-85" strokeWidth={1.8} />
              ...
              <button
                ...
                className="mr-1 inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-md text-foreground/65 opacity-0 transition-colors hover:bg-foreground/[0.06] hover:text-foreground focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring group-hover:opacity-100 data-[active=true]:opacity-100"
                data-active={active}
              >
                <X className="h-3 w-3" strokeWidth={1.8} />
              </button>
              {active ? <span aria-hidden="true" className="absolute inset-x-0 bottom-0 h-px bg-foreground/50" /> : null}
            </div>
          )
        })}
        {onCreateFile ? (
